...
public class ShopEditMembersAddAnvilMenu extends SurfShopAnvilGui {

    private final Shop shop;

    public ShopEditMembersAddAnvilMenu(@Nullable SurfShopGui parent, @NotNull Shop shop) {
        super(shop, parent, Component.text("Mitglied hinzufügen", MessageManager.PRIMARY));
        this.shop = shop;
    }
    ...
    public SurfAnvilGui copyGui() {
        return new ShopEditMembersAddAnvilMenu(getParent(), shop);
    }
    ...
    public List<AnvilRequirement> getRequirements(List<AnvilRequirement> requirements) {
        ...
        requirements.add(new AnvilMemberRequirement(shop, true));
    ...
    public List<AnvilGUI.ResponseAction> onSubmit(Player player, String input) {
        ...
        shop.addMember(offlinePlayer).thenAcceptAsync(updatedShop -> {
            player.sendMessage(MessageManager.addMember(offlinePlayer));
