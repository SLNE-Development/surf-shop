...
import dev.slne.surf.shop.server.shop.gui._2_0.edit.anvil.requirement.AnvilDescriptionLengthRequirement;
...
public class EditDescriptionAnvilGui extends SurfShopAnvilGui {
    ...
    public EditDescriptionAnvilGui(@Nullable SurfShopGui parent, Shop shop) {
        super(
                shop,
                parent,
                shop.description().map(component -> shop.getMiniMessage().serialize(component)).orElse(""),
                Component.text("Beschreibung bearbeiten", MessageManager.PRIMARY)
        );
        this.shop = shop;
    }
    ...
    public List<AnvilRequirement> getRequirements(List<AnvilRequirement> requirements) {
        requirements.add(new AnvilDescriptionLengthRequirement(shop));
